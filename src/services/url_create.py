...
def get_attempt(session, cmid):
    """
    Полный процесс старта попытки теста:
    1. GET страница теста
    2. POST первый шаг (нажатие кнопки "Пройти тест")
    3. POST preflight (нажатие кнопки "Начать попытку")
    Возвращает словарь:
    {
        "attempt_url": str,
        "attempt_id": int,
    ...
    print("Статус:", r1.status_code)

    # Проверка на popup preflight
    soup = BeautifulSoup(r1.text, "lxml")
    preflight_form = soup.find("form", id="mod_quiz_preflight_form")
    if not preflight_form:
        raise Exception("Форма preflight не найдена, возможно тест стартует без проверки")
